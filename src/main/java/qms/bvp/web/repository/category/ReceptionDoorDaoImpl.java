...
//        (select listagg(TO_CHAR(ro.name),'; ') within group (order by ro.id) as names
        List<Object[]> list=entityManager.createNativeQuery("Select rd.id,rd.name,(select ra.name from reception_area ra where ra.id=rd.reception_area) area_name,(select group_concat(ro.name ORDER BY  ro.id ASC SEPARATOR '; ') as names" +
                "  from reception_object_type ro where (ro.value & rd.reception_type_value)>0) type,rd.description,rd.disable,rd.reception_area,rd.reception_type_value from reception_door rd where rd.deleted=0 and rd.disable=0 order by rd.reception_area")
