...
app.controller('qmsCtrl',['$scope','$http','transformRequestFormPost',function ($scope,$http,transformRequestFormPost) {
        ...
        $scope.editClick=function (user) {
            $scope.item={id:user.id,username:user.username,fullname:user.fullname,description:user.description};
        ...
        $scope.addItem=function () {
            if($scope.validateAdd()){
                ...
                $http({
                    method : 'POST',
                    url : preUrl+"/admin/system/user/add",
                    data : $scope.item,
        ...
        $scope.validateAdd=function () {
            ...
            if($scope.item.password.length<6){
                $scope.messageError='Mật khẩu tổi thiểu 6 ký tự';
                return false;
            }
